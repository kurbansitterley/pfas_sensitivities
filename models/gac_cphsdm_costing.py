...
def _cost_gac(blk, parameter_blk):
    ...
    blk.onsite_thermal_regen_capital_cost = pyo.Var(
        initialize=1e5,
        units=blk.costing_package.base_currency,
        doc="Onsite thermal regeneration system capital cost",
    )
    capital_cost_expr = 0

    if blk.unit_model.config.regenerant == "onsite_thermal":

        # Capex parameters fit to data from
        # EPA-WBS bls1134 GAC Regeneration - Multiple Hearth Furnace System
        blk.onsite_thermal_cap_A = pyo.Param(
            mutable=True,
            initialize=9.79967350e4,
            units=pyo.units.USD_2021,
            # units=pyo.units.day / pyo.units.lb,
            doc="Onsite thermal regeneration capital cost coefficient A",
        )
        blk.onsite_thermal_cap_b = pyo.Param(
            mutable=True,
            initialize=0.6,
            units=pyo.units.dimensionless,
            doc="Onsite thermal regeneration capital cost exponent b",
        )

        # Energy parameters from equation EPA-WBS GAC model O&M tab, regen_energy variable (MWh/year)
        # =IF(regen_type=1,9.1217*regen_capacity^1.0475/1000,0)
        blk.onsite_thermal_power_A = pyo.Param(
            mutable=True,
            initialize=9.1217,
            # units=pyo.units.USD_2021,
        ...
        blk.onsite_thermal_power_b = pyo.Param(
            mutable=True,
            initialize=1.0475,
            units=pyo.units.dimensionless,
            doc="Onsite thermal regeneration capital cost exponent b",
        ...
        gac_usage_rate_lb_day_dim = pyo.units.convert(
            blk.unit_model.gac_usage_rate * pyo.units.day / pyo.units.lb,
            to_units=pyo.units.dimensionless,
        )

        blk.onsite_thermal_regen_capital_cost_constraint = pyo.Constraint(
            expr=blk.onsite_thermal_regen_capital_cost
            == pyo.units.convert(
                blk.onsite_thermal_cap_A
                * gac_usage_rate_lb_day_dim**blk.onsite_thermal_cap_b,
                to_units=blk.costing_package.base_currency,
            )
        )

        blk.onsite_thermal_regen_energy_consumption = pyo.Expression(
            expr=blk.onsite_thermal_power_A
            * gac_usage_rate_lb_day_dim**blk.onsite_thermal_power_b
        )

    else:
        blk.onsite_thermal_regen_capital_cost.fix(0)

    capital_cost_expr += blk.onsite_thermal_regen_capital_cost
    ...
    capital_cost_expr += blk.contactor_cost
    ...
    capital_cost_expr += blk.adsorbent_cost
    ...
    capital_cost_expr += blk.other_process_cost

    blk.costing_package.add_cost_factor(blk, "TIC")
    blk.capital_cost_constraint = pyo.Constraint(
        expr=blk.capital_cost
        == pyo.units.convert(
            blk.cost_factor * capital_cost_expr,
            to_units=blk.costing_package.base_currency,
        )
    )
    ...
    if blk.unit_model.config.hazardous_waste:
        # From EPA-WBS, haz_disposal_cost_cl
        blk.hazardous_waste_unit_cost = pyo.Param(
            mutable=True,
            initialize=363.41,
            units=pyo.units.USD_2021 * pyo.units.ton**-1,
            doc="unit cost to dispose of hazardous waste GAC adsorbent",
        )

        blk.hazardous_waste_op_cost = pyo.Expression(
            expr=pyo.units.convert(
                (
                    blk.hazardous_waste_unit_cost
        ...
        fixed_op_expr += blk.hazardous_waste_op_cost
    ...
    fixed_op_expr += blk.gac_regen_cost
    ...
    fixed_op_expr += blk.gac_makeup_cost
    blk.fixed_operating_cost_constraint = pyo.Constraint(
        expr=blk.fixed_operating_cost == fixed_op_expr
    )
    ...
    tot_power += pyo.units.convert(blk.energy_consumption, to_units=pyo.units.kW)

    if blk.unit_model.config.regenerant == "onsite_thermal":
        tot_power += pyo.units.convert(blk.onsite_thermal_regen_energy_consumption, to_units=pyo.units.kW)
    ...
    if blk.unit_model.config.regenerant == "custom":
        ...
        blk.regen_frac_solvent = pyo.Param(
            initialize=1.0,
            mutable=True,
            units=pyo.units.dimensionless,
            doc="fraction of GAC bed regenerated with solvent",
        )

        for chem, ww in blk.unit_model.config.regen_composition.items():

            blk.gac_regen_cost.fix(0)
            blk.gac_regen_cost_constraint.deactivate()
            # parameter_blk.regen_frac.fix(0.5)

            # blk.costing_package.cost_flow(blk.unit_model.regen_flow_mass * ww, chem)
            blk.costing_package.cost_flow(
                blk.unit_model.regen_flow_mass * ww * blk.regen_frac_solvent, chem
            )
            # blk.costing_package.cost_flow(blk.unit_model.regen_flow_mass * ww * parameter_blk.regen_frac, chem)
